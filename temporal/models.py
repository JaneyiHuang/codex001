...
class TemporalActor(nn.Module):
    """
    Actor with an additional repeat head for temporal distillation.

    Output:
        action_logits: shape (..., n_actions)
        repeat_raw: shape (...,), a continuous value later passed through ReLU.
    """
    def __init__(self, obs_dim: int, n_actions: int, hidden_dims: list[int] | None = None):
        super().__init__()
    ...
    def predict_temporal(
        ...
        repeats = torch.round(F.relu(repeat_raw) * repeat_scale).to(torch.int64)
        repeats = torch.clamp(repeats, min=0, max=max_repeat)
